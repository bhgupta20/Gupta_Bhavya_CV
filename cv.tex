...
\begin{tabular}{lllll}
    ...
    \faIcon{orcid} \href{https://orcid.org/0009-0005-4810-1454}{0009-0005-4810-1454} \\
\end{tabular}
\begin{tabular}{l}
...
\textit{Accelerated AI Algorithms for Data Driven Discovery (A3D3) Post-baccalaureate Fellow}
\begin{itemize}
    \item Working under Prof. Erik Katsavounidis, I am developing machine-learning frameworks to improve the low-latency detection and parameter estimation of binary neutron star (BNS) mergers in LIGO-Virgo gravitational-wave data.
    \item Extending the Aframe and AMPLFI pipelines to enhance sensitivity to low-SNR inspiral signals through domain-adaptive data augmentations, neural likelihood inference, and optimized architectures for lower-mass compact binaries.
    \item Strengthening expertise in time-domain astrophysics, real-time inference, and large-scale data processing, enabling earlier identification and sky localization of BNS mergers for multi-messenger astronomy.
...
\begin{itemize}
    \item Worked under Prof. Javier Duarte and Prof. Tongyan Lin to model the gravitational interactions between dark matter (DM) subhalos and stellar streams, using perturbations such as gaps and spurs to infer subhalo dynamical properties and constrain the DM subhalo mass function.
    \item Adapted \href{https://academic.oup.com/mnras/article/450/1/1136/1008580}{Erkal (2015)’s} analytic framework for velocity perturbations to generate and evolve synthetic streams with \texttt{Gala} and \texttt{Galpy}, validating that simulated morphologies and kinematics reproduce analytic expectations.
    \item Developed a pipeline to recover stream–subhalo impact parameters (mass, velocity, time of encounter) using Markov Chain Monte Carlo (MCMC) for streams with tractable likelihoods.
    \item Extended this framework to complex, non-analytic cases using likelihood-free inference, implementing and training Graph Neural Networks (GNNs) and Normalizing Flows (nflows) on distributed CPU/GPU clusters at the San Diego Supercomputer Center.
\end{itemize}\eatvspace
}
\end{datetabular}
...
\dateentry{}{
\begin{itemize}
    \item Compared traditional MCMC with Simulation-Based Inference (SBI), finding that MCMC achieved tighter posteriors, while ML-based methods offered faster, scalable inference across large simulated datasets, further highlighting how ML-driven inference can reveal faint DM substructure signatures in upcoming Rubin–LSST data.
...
\begin{itemize}
    \item Worked with Professor Alison Coil to investigate the kinematic properties of outflows from accreting supermassive black holes (SMBHs) that drive galactic evolution.
...
\begin{itemize}
    \item Worked with Carnegie Staff Astronomer Dr. Peter Senchyna to study metal-poor, star-forming dwarf galaxies (z < 1) as local analogs of high-redshift systems (z > 6), probing interstellar medium (ISM) conditions relevant to early galaxy formation.
    \item Investigated the distribution of highly ionized gas by analyzing [O III] and [C II] far-infrared emission lines from SOFIA spectroscopic data cubes using the SOSPEX package for flux extraction and line diagnostics.
    \item Compared [O III]/[C II] ratios and emission morphology with star formation and metallicity indicators, benchmarking results against the Dwarf Galaxy Survey (DGS) to assess ionized gas properties.
    \item Found that, unlike high-redshift systems observed with ALMA, local analogs show strong low-ionization [C II] emission from photodissociation regions (PDRs), indicating structural differences in the ISM.
...
\begin{itemize}
    \item Worked under Professor Pierre Gourdain and utilized scientific Python packages to analyze the collisions in plasma and investigate smooth particle hydrodynamics, magnetohydrodynamics, and single-particle motion.
...
\begin{datetabular}{2em}
\dateentry{2025}{
\textit{NSF HDR Ecosystem Conference 2025 Oral Presentation}
\begin{itemize}
...
\begin{datetabular}{2em}
\dateentry{2025}{
\textit{NSF HDR Ecosystem Conference 2025 Poster Presentation}
\begin{itemize}
    \item \href{https://drive.google.com/file/d/1wVuC0gjLqMzcLyqCxpdk40GgmrdvKOll/view?usp=sharing}{``Accelerating Gravitational Wave Astronomy with Machine Learning''}
\end{itemize}\eatvspace}
\end{datetabular}
...
\begin{datetabular}{2em}
\dateentry{2025}{Accelerated AI Algorithms for Data-Driven Discovery (A3D3) Post-baccalaureate Fellowship
\begin{itemize}
...
\begin{datetabular}{2em}
\dateentry{2021-24}{Pratibha Scholarship
\begin{itemize}
    \item The Aditya Birla Group (ABG) Employees’ Children’s Scholarship Plan was started in 2004. The scholarship program has received over 2400 applications from across the world. Awarded \$92,000 in scholarship over 3 years for my undergraduate degree.
\end{itemize}\eatvspace
}
\end{datetabular}
...
\begin{itemize}
    ...
    \item[ \small{\faCenter{envelope}}] \href{mailto:deep1018@mit.edu}{deep1018@mit.edu} 
\end{itemize}\eatvspace}
\end{datetabular}
